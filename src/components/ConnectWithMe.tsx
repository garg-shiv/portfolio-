...
const platformMeta: Record<
  string,
  { icon: React.ReactNode; color: string; name: string; getUrl?: () => string }
> = {
  linkedin: {
    name: 'LinkedIn',
    icon: <FaLinkedin />,
...
function ConnectWithMe({}) {
  ...
  const renderSocialLinks = () => {
    return socials.map((social, index) => {
      ...
      const rgbColor = getRgbValues(meta.color);
          ...
          >
            <div className="absolute inset-0 p-[1px] rounded-2xl overflow-hidden opacity-0 group-hover:opacity-100 transition-opacity duration-500">
              <div
                ...
                style={{
                  background: `linear-gradient(45deg, ${meta.color}80, rgba(${rgbColor}, 0.1), ${meta.color}80)`,
            ...
            <div className="relative flex flex-col items-center text-center p-6 h-full z-10">
              ...
              >
                <div style={{ color: meta.color }} className="text-2xl">
                  {meta.icon}
                </div>
              </div>

              <span className="text-l font-medium text-white mb-1">{meta.name}</span>
            </div>
          </a>
        </motion.div>
      );
